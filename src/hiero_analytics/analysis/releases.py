...
_STALENESS_COLUMNS = ["repo", "latest_release", "days_since_last_release"]
...
def build_release_staleness(
    ...
    *,
    ...
    latest = releases.groupby("repo")["published_at"].max().rename("latest_release")
    staleness = repo_universe.merge(latest, on="repo", how="left")
    staleness["latest_release"] = pd.to_datetime(staleness["latest_release"], utc=True)
    days = (now - staleness["latest_release"]).dt.days
    staleness["days_since_last_release"] = days.astype("Int64")  # nullable — NaT stays null, not 0
    return staleness[_STALENESS_COLUMNS].reset_index(drop=True)
